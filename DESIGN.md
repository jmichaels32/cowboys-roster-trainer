...
Session setup is one editable sentence: “Practice [level] with [content] for [length].” Each emphasized value opens a focused bottom sheet; the screen shows no setup grids, group label, deck description, or practice explanation. Mastery fixes content to all facts. The only persistent action is “Start [length].” Keep the recipe card self-contained: it contains only the editable sentence. Package/topic, level, content, length, and the active study type persist separately for every deck until the user changes them; progress must never replace those choices with an automatic recommendation. On Results, both the back arrow and `Change practice` return directly to the preserved setup; only the setup back arrow returns to Decks. One muted, centered line beneath the primary action remembers the exact deck, study type, package/topic, level, and applicable content combination as `x sessions · y% correct`, or `Not practiced yet`; length never creates a separate history. A session counts after its first submitted answer, including partial sessions, and accuracy includes every submitted answer. Do not turn this into a card, chart, streak, date, or stats destination. Mastery progress replaces the existing item count inside the package/topic sheet as `x of y mastered`; a completed group reads `✓ Mastered` and remains available for review. Never add a persistent mastery panel, badge, or roster-row status for the same information. Player groups count fully verified players. Lineup and Trivia groups count questions last answered correctly through typed recall; recognition alone never marks mastery.
...
Team-roster player facts are face/name, jersey number, roster position, and college. NFL Top 100 player facts are face/name, current team, position, and rank. A normal session selects distinct players from the chosen package and asks one chosen fact per player. Scheduling treats each player-and-fact pair as a separate combination: unattempted combinations come first, spread across the least-practiced players; after exposure, combinations run from lowest accuracy to highest, with least-recently seen breaking ties. Recognition and typed recall retain separate histories. Mastery asks all four deck-specific facts for each selected player, interleaves players and fact types, and reveals only the answer to the current fact. It never exposes the player's other quiz facts before their questions are complete. Typed answers tolerate harmless case, accent, punctuation, apostrophe, and number-sign differences plus explicit safe aliases such as name suffix omission, full position/team names, and common college names; they do not use fuzzy spelling. A valid typed match grades immediately without requiring submit. Otherwise `Check answer` remains available and becomes the single `Correct` or `Incorrect` status after grading; feedback must not repeat that status. Each newly focused field is positioned just above the mobile keyboard. Every team-player practice photo carries one compact `experience · height · weight` line directly beneath it so those passive attributes become part of the player's visual association; they remain untested reference attributes. Roster status and depth-chart position remain roster-browser reference data unless the product model is explicitly expanded later.
