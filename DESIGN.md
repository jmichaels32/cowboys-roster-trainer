...
Lineup questions are generated from the bundled roster and published depth fields so a roster refresh also refreshes answers. Multi-player questions accept every required player in any order unless the prompt explicitly asks for depth-chart order. Trivia facts live in a separate bundled data file. Every trivia question must resolve to an authoritative source title and URL, carry a verification date, and include a short paraphrased evidence note. The trivia validator blocks missing citations, invalid answer aliases, duplicate distractors, and empty packs. Both modes work fully offline after installation.

Trivia questions are text-first. Do not insert a generic category badge, logo, or filler illustration between the prompt and answers. A historical photo may appear only after the answer is revealed, when it depicts the specific person or event, has a verified source and usage basis, and is bundled for offline use. Lineup may retain compact position markers such as `QB1` or `LT` because those labels carry answer-relevant information.
