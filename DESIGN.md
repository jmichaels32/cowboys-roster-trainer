...
Trivia questions are text-first. Do not insert a generic category badge, logo, or filler illustration between the prompt and answers. The verified historical image appears only after the answer is revealed, paired with a short caption that explains its connection to the answer. Prefer the exact person or event; if an exact event image is unavailable, use a directly related subject and state that relationship honestly in the caption. Images are downloaded once, optimized as local WebP assets, and never fetched during practice. Lineup may retain compact position markers such as `QB1` or `LT` because those labels carry answer-relevant information.
