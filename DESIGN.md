...
Trivia questions are text-first. Do not insert a category eyebrow, generic badge, logo, or filler illustration between the prompt and answers. Recognition answers use flat full-width rows rather than a stack of rounded mini-cards; after grading, retain only the correct answer and the selected wrong answer so the reveal reaches the user quickly. The verified historical image appears only after the answer is revealed. Visible image copy is limited to a two-to-four-word subject label; longer provenance, relationship notes, credits, and alt text remain in the bundled data rather than becoming on-screen description. Prefer the exact person or event; if an exact event image is unavailable, use a directly related subject and record that relationship honestly in metadata. Images are downloaded once, optimized as local WebP assets, and never fetched during practice. Lineup may retain compact position markers such as `QB1` or `LT` because those labels carry answer-relevant information.
